...
OrderForm.prototype._ajax = function (formData) {
	grecaptcha.ready(() => {
		grecaptcha.execute('6LdwdgcaAAAAAJSaaQWZwUcvdXPxKvBXZ0iRjSb3', {action: 'checkout'}).then((token) => {
			let recaptchaResponse = document.getElementById('recaptchaResponse');
			recaptchaResponse.value = token;
		  // Add your logic to submit to your backend server here.
			formData.delete('agreement');
			formData.delete('delivery');
			var order = { customer: Object.fromEntries(formData), recaptcha_response: recaptchaResponse.value };
			$.ajax({
				url: "/cart/checkout",
				type: "POST",
