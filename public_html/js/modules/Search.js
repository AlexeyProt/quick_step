...
class Search {
	constructor(container) {
		...
		/**
		 * Выполняется запрос при пустом значении input, если true.
		 *
		 * @var boolean
		 */
		this.isAllowEmptyInput = false;
		this.searchDataURL;
		this.postData;
	...
	*/
	...
	async loadSearchData() {
		...
		if (!this.isAllowEmptyInput && !this.input.value) return [];
		
		this.progressAnimation.start();
		
		let postData = (this.postData) ? this.postData : {searchQuery: this.input.value};
