...
tinymce.PluginManager.add('testplugin', function(editor, url) {
	...
	editor.on('ExecCommand', function (e) {
		...
		if ( end.isContentEditable == false ) {
			while ( end.contentEditable != 'false' ) {
				editor.selection.select(end.parentNode);
				end = editor.selection.getEnd();
			}
		...
		var nonEditable = parent.querySelectorAll(('[contenteditable="false"]'));
		if (nonEditable.length == 0) return;
		for ( var i = 0; i < nonEditable.length; i++ ) {
			nonEditable[i].contentEditable = true;
		}		
		console.log('s');
		...
		for ( var i = 0; i < nonEditable.length; i++ ) {
			nonEditable[i].contentEditable = false;
		}			
